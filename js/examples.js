...
const setDelTheme = (event) => {
  delThemeList.setDelBtnTheme(
    delThemeList.delBtnThemes[event.target.getAttribute("theme")]
  );
};
...
validateOne.addValidation((value) => {
  return (
    ...
    (value.match(/\d{2}\/\d{2}\/\d{4}/) &&
      value.match(/\d{2}\/\d{2}\/\d{4}/)[0] === value) ||
...
const handleSubmit = (event) => {
  ...
  const items = event.target.firstElementChild.value
    .split(" ")
    .filter((val) => val !== "");
...
const handleGetItems = (event) => {
  ...
  p.appendChild(
    document.createTextNode(
      `The items given in the list are: ${data.toString()}`
    )
  );
};
...
markedLists.element.addEventListener("mousedown", (event) => {
  if (
    ...
    markedLists.element.insertBefore(
      ulElement,
      markedLists.element.children[0]
    );
