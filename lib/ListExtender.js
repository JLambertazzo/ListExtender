...
(function (global, document) {
  ...
  function ListExtender(options = {}) {
    ...
    this.element = this.options.isUnordered
    ...
    this.element.addEventListener("keypress", (event) => {
      if (event.key === "Enter" && !canRemove(event.target, this)) {
        if (event.target.parentElement.nextSibling) {
          // if list has not reached maxlength
          try {
    ...
    this.element.addEventListener("dblclick", (event) => {
      if (
        ...
        !(
          ...
          customChecks(document.activeElement, this)
        )
      ) {
      ...
      if (
        ...
        if (["text", "password", "url"].includes(this.attr.type)) {
          event.target.firstElementChild.selectionStart =
            event.target.firstElementChild.selectionEnd;
        }
      }
    });
    ...
    this.element.addEventListener("input", (event) => {
      ...
      const inputs = [...this.element.querySelectorAll("input")].filter(
        (element) => element.getAttribute("type") !== "submit"
      );
      for (let i = 0; i < inputs.length; i++) {
        if (!customChecks(inputs[i], this) || !inputs[i].checkValidity()) {
          return;
        }
      }
    ...
    */
    if (this.options.allowReorder) {
      this.element.addEventListener("dragover", (event) => {
        ...
        const children = [...this.element.children].filter(
          (el) => el !== dragging
        );
    ...
    if (this.options.showDeleteButton) {
      this.element.addEventListener("mouseenter", (event) => {
        ...
        children.forEach((child) => {
          if (child.querySelector('input[type="submit"]')) {
            child.querySelector('input[type="submit"]').style.visibility =
              "visible";
          }
        });
      });
      ...
      this.element.addEventListener("mousedown", (event) => {
        if (
          ...
          !this.options.allowStrikethrough
        ) {
          ...
          let isStruck =
            selectedElement.style.getPropertyValue("text-decoration") ==
            "line-through";
          if (isStruck) {
            this.element.removeChild(selectedElement);
            let listElements = [...ulElement.getElementsByTagName("li")];
            listElements.map((el, i) => el.setAttribute("key", i));
            this.listSize--;
            if (inputCount(this) < 1) {
  ...
  function turnToList(input, listObj) {
    ...
    listObj.setDelBtnTheme(listObj.delBtnThemes.default);
  }
  ...
  function canRemove(input, listObj) {
    ...
    return (
      input.value === "" &&
      (inputCount(listObj) >= 2 || listObj.listSize > listObj.maxSize)
    );
  }
  ...
  function inputCount(listObj) {
    ...
    return children.reduce((numInputs, child) => {
      if (
        ...
        child.firstElementChild.getAttribute("type") !== "submit"
      ) {
        return ++numInputs;
      }
      return numInputs;
  ...
  function handleDragStart(event) {
    if (
      ...
      event.target.firstElementChild.getAttribute("type") === "text"
    ) {
      event.preventDefault();
    } else {
  ...
  ListExtender.prototype = {
    ...
    addFromArray: function (data) {
      ...
      if (
        this.element.lastElementChild.firstElementChild &&
        this.element.lastElementChild.firstElementChild.getAttribute("type") !==
          "submit" &&
    ...
    Python: {
      background: "#8fce00",
      color: "#dbc01e",
      "font-family": "Poppins,Open Sans,Montserrat",
      "font-weight": "bold",
      "list-style": "'\\1F40D'",
    },
