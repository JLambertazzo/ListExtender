    ...
    */
    ...
    if (this.options.showDeleteButton) {
      ...
      this.element.addEventListener('mousedown', (event) => {
        if (event.target.tagName === 'INPUT' && event.target.getAttribute('type') === 'submit' && !this.options.allowStrikethrough) {
        ...
        } else if (event.target.tagName === 'INPUT' && event.target.getAttribute('type') === 'submit' && this.options.allowStrikethrough) {
          let ulElement = event.target.parentElement.parentElement;
          let selectedElement = event.target.parentElement;
          let isStruck = selectedElement.style.getPropertyValue('text-decoration') == 'line-through';
          if (isStruck) {
